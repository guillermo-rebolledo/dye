...
`scan` cannot put an inversion back — the value it would invert is already a
positive. Nothing in the branch reaches the Print stage, which is still
unimplemented for everything.
...
`SpectralModel.project` reads the transparency the way a viewer does: form the
...
Velvia bakes 65³ Colour Cubes rather than 33³, and Provia does not. A slide's
...
- The third stage card offers no scan-or-print choice for a reversal Stock. It
  ...
  belong to the frame and the lens rather than to the Process.
