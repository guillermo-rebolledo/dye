...
- `halation`: the whole point. Vision3 500T's rem-jet backing absorbs the light
  ...
  datasheet at all. They are tuned so a practical light in frame reads as the
  glowing red halo this Stock is known for, and the Provenance says so.
- `nominalISO` 800: the Box Speed CineStill prints, and the only measured claim
  this file makes.
- `trueISO` 500: the parent Emulsion's Exposure Index. The parent marks its own
  ...
  marked **artistic**, because what a rem-jet-free ECN-2 emulsion actually meters
